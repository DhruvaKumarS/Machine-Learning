...
def preprocess():
    ...
    del_cols = np.concatenate((train_preprocess, validation_preprocess, test_preprocess), axis=0)

    del_list = np.all(del_cols == del_cols[0,:], axis=0)
    dl = []
    for i in range(len(del_list.tolist())):
        if(del_list[i] == True):
            dl.append(i)
    selected_features = np.delete([i for i in range(train_data.shape[1])], dl)
    test_data = np.delete(test_data, dl, axis=1)
    validation_data = np.delete(validation_data, dl, axis=1)
    train_data = np.delete(train_data, dl, axis=1)
...
def nnObjFunction(params, *args):
    ...
    training_label_matrix = np.zeros(shape=(training_label.shape[0], output_layer_output.shape[1]))
    training_label = training_label.astype(int)
    training_label_matrix[np.arange(len(training_label_matrix)), training_label] = 1
    
    error_output = -1 * np.sum(training_label_matrix * np.log(output_layer_output) + (1 - training_label_matrix) * np.log(1 - output_layer_output), axis=1)
    error = np.sum(error_output, axis=0)/error_output.shape[0]
